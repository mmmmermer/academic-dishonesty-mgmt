...
def _parse_import_row(row):
    """解析导入行，返回 (sid, name, major, reason, punishment_date)；学号为空时返回 None。"""
...
def _upsert_one_blacklist(db, sid, name, major, reason, punishment_date):
    """插入或更新一条名单记录，返回 'imported' 或 'updated'。"""
...
def _run_batch_import(db, df, filename):
    """执行批量导入循环，成功返回结果 dict，异常时 rollback 并返回 None。"""
    imported = 0
    updated = 0
    skipped = 0
    skipped_rows = []
...
def _handle_import_confirm(db):
    """执行「开始导入」逻辑：调用批量导入，更新 session 并展示结果或错误。成功时返回 True。"""
    ...
    if result is None:
        st.error(
            "导入失败，已成功导入部分数据；后续数据出错，请检查 Excel 格式（需包含：姓名、学号、专业、原因、处分时间）。"
        )
        return False
...
def _try_manual_add(db, add_name, add_student_id, add_major, add_reason, add_date):
    """执行手动新增逻辑，成功返回 True 并已展示成功/失败提示，失败返回 False。"""
...
def _render_effective_filters_ui():
    """渲染生效名单筛选输入，返回 (姓名, 学号, 专业)  stripped。"""
    ...
    return (filter_effective_name or "").strip(), (filter_effective_sid or "").strip(), (filter_effective_major or "").strip()


def _render_effective_display_options():
    """渲染生效名单「显示与排序」展开区，返回 (page_size, sort_key, sort_asc)。"""
...
def _try_save_edit_form(edit_db, rec, edit_id, edit_name, edit_major, edit_reason, edit_date):
    """执行编辑表单保存逻辑，成功返回 True 并已 rerun，失败返回 False。"""
    try:
...
def _render_revoked_filters_ui():
    """渲染已撤销名单筛选输入，返回 (姓名, 学号, 专业) stripped。"""
    ...
    return (filter_revoked_name or "").strip(), (filter_revoked_sid or "").strip(), (filter_revoked_major or "").strip()


def _render_revoked_display_options():
    """渲染已撤销名单「显示与排序」展开区，返回 (page_size, sort_key, sort_asc)。"""
...
def _try_add_user(db, uname, pwd, fname, new_role):
    """执行新增用户逻辑，成功返回 True，失败返回 False。调用方在 True 时需执行 st.rerun()。"""
