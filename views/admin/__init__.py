...
"""
from views.admin.list_query import _render_list_query
...
ADMIN_NAV_OPTIONS = ["› 名单查询", "› 名单管理", "› 系统维护", "› 用户管理"]
NAV_QUERY, NAV_MANAGEMENT, NAV_SYSTEM, NAV_USER = 0, 1, 2, 3
ADMIN_NAV_KEY = "admin_nav_radio"


def _get_admin_nav_index():
    """从 session_state 读取当前选中的板块索引（唯一数据源）。"""
    val = st.session_state.get(ADMIN_NAV_KEY, ADMIN_NAV_OPTIONS[NAV_QUERY])
...
def render_admin_sidebar_nav():
    ...
    if ADMIN_NAV_KEY not in st.session_state:
        st.session_state[ADMIN_NAV_KEY] = ADMIN_NAV_OPTIONS[NAV_QUERY]
    st.markdown("### 管理员")
    ...
    st.radio(
        ...
        on_change=_on_admin_nav_change
    )
