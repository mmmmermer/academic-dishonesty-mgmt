...
from core.file_safe_guard import safe_filename, remove_old_pdf, _PDF_DIR, validate_pdf_upload, generate_pdf_filename, save_pdf_file
    ...
    with st.container(border=True):
        ...
        with action_cols[2]:
            ...
            st.download_button(
                ...
                use_container_width=True,
                type="primary",
                on_click=lambda c=cart_count: log_audit_action(AUDIT_EXPORT, target="导出暂存区", details=f"共导出 {c} 条数据")
            )



@st.dialog("修改学术失信人员记录", width="large")
def _show_edit_dialog(edit_id):
